...
data = read.csv("UK_GDP_1955_2019.csv", sep = ";", header = TRUE)
...
result.der2 = semifarima.der(result, lgdp, nu = 2, mse.RANGE = 0.05, kn = 4, bb = 1)
...
plot.trend <- ggplot(df, aes(x = year, y = lgdp)) + 
  ...
  labs(title = "(a) Log of UK-GDP & estimated trends", y = "Log-GDP and trends") + 
  scale_x_continuous(name = "", breaks = seq(1955, 2020, 10)) +
...
plot.res <- ggplot(df, aes(x = year, y = res)) +
  ...
  scale_x_continuous(name = "Year", breaks = seq(1955, 2020, 10)) +
...
ggsave("UKgdp.pdf", height = 5, width = 9.5, dpi = 600)
