...
###################################other adjustments#################################################################################
semifarima_pq.lpf = function(x, p.min = 0, p.max = 1, q.min = 0, q.max = 1, mse.RANGE = 0.025,b0 = NULL, pg = 3, kn = 2, bb = 0, bbd = 1, IM = "E", IF = 1)
{
  alpha.SEMI<-0.01
  alpha.MLE<-0.05
  ...
  for(iITER in 1:nITER)
  {
    ...
    if( icrit==0 )
    {
      ...
      ############ estimation of theta ##########
        ...
        result<-mle.farima.new(x.DETRENDED, p.min, p.max, q.min, q.max, 0.05, 0)
      ...
      if(convc==1){
        ...
        b2<-min(0.49,b2) 
        #browser()
        gk<-smooth.lpf(x, pg+1, pg+2, kn, b2, bbd)
...
################################mle.farima###############################################
mle.farima.new <- function(x, p.min, p.max, q.min, q.max, alpha, iCI)
{
  ndata<-length(x)
  result<-mle.farima(x, p.min, p.max, q.min, q.max)
...
#######################################mle.farima################################### 
mle.farima <- function(x, p.min, p.max, q.min, q.max)
{ 
  #eta <- (1:(p.min+q+1))*0
  #
  r <- x
  ...
  ###############fracdiff######################
  BIC.mat <- matrix(NA, p.max-p.min+1, q.max-q.min+1)
  options(warn = -1)
  for (i in 0:(p.max-p.min)){
    for (j in 0:(q.max-q.min)){
      M <- fracdiff::fracdiff(r, nar = i, nma = j, M = 100, drange = c(0, 0.5))
      BIC.mat[i+1, j+1] <- -2 * M$log.likelihood + log(n) * (i + j)
    }
  }
  BIC.opt = min(BIC.mat)
  opt.ord <- which(BIC.mat == min(BIC.mat), arr.ind = TRUE) - 1
  p.BIC = opt.ord[1] + p.min
  q.BIC = opt.ord[2] + q.min
    result <- fracdiff::fracdiff(r, nar = p.BIC, nma = q.BIC, M=100, drange = c(0, 0.5))
    options(warn = 0)
  ...
  drop(list(x = x, p=p.BIC, q=q.BIC, etaEST=etaEST, BIC.opt = BIC.opt))
}
##########################################smooth.lpf################################
