semifarima.der<-function(result, x, nu, mse.RANGE, kn, bb) # SL nu
{
  #########The data
  x<-x
  ...
  if(nu == 2){
    Kk<-72 #### Kk=(factorial(p+1))^2/(2*(p+1))
    if(kn==1){
      Bk<-(7/12)^2                  
      Rk<-22.5}
    if(kn==2){
    ...
    if( icrit==0 )
    {
      ...
      if(delta!=0)
      {
        ...
        const1<-(Bk*Kk*Rk*(1-2*mse.RANGE)
                 *(2+1-2*delta))**(1/(2*(nu+2)+1-2*delta))
      }
      else
      {
        const1<-(Bk*Rk*(1-2*mse.RANGE)*(2+1)*2*pi)**(1/(2*(nu+2)+1))
      }
