...
###################################other adjustments#################################################################################
semifarima.lpf = function(x, p, q, mse.RANGE,b0 = NULL, pg, kn, bb, IF)
{
  alpha.SEMI<-0.01
  alpha.MLE<-0.05
...
{ 
  ...
  else
  {
    ...
    else 
    {
      ...
      eta<-c(result$d,result$ar,result$ma)
      z.AR<-abs(polyroot(c(1,-result$ar)))
      z.MA<-abs(polyroot(c(1,result$ma)))
    }
  } 
  ...
  if( (abs(eta[1]-0.5)<0.01)||(abs(eta[1]-(-0.5))<0.01)||
      (min(z.ARMA)<= 1.05))
  { 
    loglik<-result$log.likelihood-10**10 
  }  
  etaEST<-eta      
  loglikEST<-loglik  
