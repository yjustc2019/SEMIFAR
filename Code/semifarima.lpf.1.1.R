...
semifarima.lpf = function(x, p = 1, q = 1, mse.RANGE = 0.025,b0 = NULL, pg = 3, kn = 2, bb = 0, bbd = 1, IM = "E", IF = 1, par.est = "fracdiff")
{
  ...
  for(iITER in 1:nITER)
  {
    ...
    if( icrit==0 )
    {
      ...
      if(iITER==1) 
      {
        result<-mle.farima.new(x.DETRENDED, p, q, 0.05, 0, par.est)  
...
################################mle.farima###############################################
mle.farima.new <- function(x, p, q, alpha, iCI, par.est)
{
  ndata<-length(x)
  
  p = p
  q = q
  result<-mle.farima(x, p, q, par.est)
...
#######################################mle.farima################################### 
mle.farima <- function(x, p, q, par.est)
{ 
  eta <- (1:(p+q+1))*0
  loglik <- 0
  r<-x
  ...
  if(par.est == "fracdiff")
  {
    result=fracdiff::fracdiff(r,nar=p, nma=q,M=100,drange = c(0,0.5))
