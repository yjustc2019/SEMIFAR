                          ...
                          mse.RANGE, pg, kn, bb)
{
  ...
  # if estimated m=0 and estimated bandwidth too small, then 
  # m must be equal to one (with probability one asymptotically).
  # Hence: carry out SEMIFAR estimation for differenced data with mmax=0.
  
  n<-length(x)
  b.crit<-n**(-5/7) + 1/n
  if( (result$b0BIC <= b.crit) & (result$mBIC==0) )
  {
    x<-diff(x)
    result<-mle.semifar.bic.alg2(x,pmin.SEMI,pmax.SEMI,qmin.SEMI,qmax.SEMI,0,
                                 mse.RANGE, pg, kn, bb)
    result$mmax.SEMI<-1
    result$thetaBIC[2]<-result$thetaBIC[2]+1
    result$etaBIC[1]<-result$etaBIC[1]+1
    result$dBIC<-result$dBIC+1
    result$mBIC<-1
    result$xBIC<-xinput.SEMI
  }
                               ...
                               bb)
{
  ...
  for(iITER in 1:nITER)
  {
    ...
    x.DATA<-xinput.SEMI                           
    if( (mBIC.1==1)||((iITER==1)&(mmax.SEMI>0)) ) 
    {x.DATA<-diff(x.DATA)}                      
    n.DATA<-length(x.DATA)
    ti<-(1:n.DATA)/n.DATA                        
    ...
    if( icrit==0 )
    {
      ...
      #------- estimate g
      if(iITER<=1)
      {g0<-smooth.lpf(x.DATA, 0, pg, 1, b0, bb)
      ...
      if(iITER==2)
      {
        ...
        q<-qmin.MLE
        m<-0
        result0<-mle.far.bic(x.DETRENDED,0,p,p,q,q,0.05,0) 
        result<-result0
        
        # estimation for m=1 with p=0,
        # decision between m=0 and m=1
        #browser()
        if(mmax.SEMI>0)
        {
          ti.1<-(1:(n.DATA-1))/(n.DATA-1)
          g0.1<-smooth.lpf(diff(x.DATA), 0, pg, 1, b0, bb) 
          
          x.DETRENDED.1<-diff(x.DATA)-g0.1
          
          result1<-mle.far.bic(x.DETRENDED.1,0, 
                               p,p,q,q,0.05,0)
          
          if(result1$BIC.opt < result$BIC.opt)
          {
            result<-result1
            m<-1 
          }
        ...
        if(pmax.MLE>pmin.MLE)
        {
          for(p in (pmin.MLE+1):pmax.MLE) 
          {
            m.p<-0
            result0<-mle.far.bic(x.DETRENDED,0, 
                                 p,p,q,q,0.05,0)
            if(mmax.SEMI>0)
            {
              result1<-mle.far.bic(x.DETRENDED.1,0, 
            ...
            # estimation for m=1 with given p,
            # decision between m=0 and m=1
            
            if(mmax.SEMI>0)
            {
              if(result1$BIC.opt < result0$BIC.opt) # if 
              {
                result.p<-result1
                m.p<-1 ##########SL
              }
              if(result.p$BIC.opt < result$BIC.opt)
              {
                result<-result.p
                m<-m.p
              }
      ...
      #---------- 2 -------- Cf
      #if(pBIC>0)
      #if(pBIC>0 && qBIC>0) ######### Fälle abändern
      #{
        #browser()
        ar.PARAMETERS<-list(order=pBIC,
                            #ar=thetaBIC[c(-1,-2)],
                            if(pBIC > 0){ar = thetaBIC[3:(3 + (pBIC-1))]}else{ar = 0}, ####SL
                            if(qBIC > 0){ma = thetaBIC[(-1 : -(pBIC+2))]}else{ma = 0}, ####SL
                            var.pred=thetaBIC[1])
        # f<-spec.ar1(ar.PARAMETERS,n.freq = 1000, plot=FALSE)
        # f<-f$spec
        # Cf<-f[1]/(2*pi) ##################Cf neu berechnen mit MA
        Cf <- sum(c(1, -ar.PARAMETERS[[3]]))^2 / (1 - sum(ar.PARAMETERS[[2]]))^2 * ar.PARAMETERS$var.pred / (2*pi) ###SL
      #}
      #else
      #{
       # Cf<-thetaBIC[1]/(2*pi) ###############Cf neu berechnen mit MA
      #}
      ...
      if(convc==1){                    
        ...
        b0<-max(n.DATA**(-5/7),b0) # nicht kleiner als 0.005
      }  
...
mle.farima.bic<-function(xinput,mmax,pmin,pmax,qmin,qmax,alpha,iCI)
{
  ...
  # BIC-matrix and optimal choice
  
  BIC<-matrix(0,nrow=pmax-pmin+1,ncol=qmax-qmin+1)
  ...
  else
  {
    ...
    sigma2BIC<-arima(r,order=c(pBIC,mBIC,qBIC))$sigma2            
...
######################################mle.far.bic############################################
mle.far.bic<-function(xinput,mmax,pmin,pmax,qmin,qmax,alpha,iCI)
{      
  #browser()
  ndata<-length(xinput)
  ...
  BIC<-matrix(0,nrow=pmax-pmin+1,ncol=1)
  ...
  {
    result<-ar.burg(r,order.max=pBIC)
    sigma2BIC<-result$var.pred
    rBIC<-result$residuals
  }
...
####################################spec.ar#######################################SL#
spec.ar1<-function (x, n.freq, order = NULL, plot = TRUE, na.action = na.fail, 
                    method = "yule-walker", ...) 
{
  if (!is.list(x)) {
  ...
  if (nser == 1) {
    ...
    spec <- if (order >= 1) {
      ...
      var.p/(xfreq * ((1 - cs)^2 + sn^2)) #xfreq=1
    }
